...
//Format improvements and sketck fixes by Manuel Forcén 2021/10/22
//Owen Duffy 20190905
...
uint8_t id[8]; //array to store the Ibutton ID.
uint8_t id2[8]= {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
...
void addCmdArg(char* buffer, uint8_t bufferLength) {
    uint8_t count = countCmdArgs();
...
uint8_t splitCmdLineArgs(char* buffer, uint8_t bufferLength) {
    ...
    while (arg_index_end < bufferLength)
    {
        ...
        if ((buffer[arg_index_end] == ' ') || (arg_index_end + 1 == bufferLength))
        {
            ...
            if (arg_index_end + 1 == bufferLength) arg_index_end = bufferLength;
...
//function to write ID uint8_ts with RW1990 (special timing, inversion)
int writeuint8_t_rw1990(uint8_t data) {
    int data_bit;
